...
void emul_ppc_save_state(emul_ppc_state *state)
{
    state->ram = ram;
    state->ram_size = ram_size;
    memcpy(state->r, r, sizeof(state->r));
    memcpy(state->fr, fr, sizeof(state->fr));
    state->lr = lr;
    state->ctr = ctr;
    state->cr = cr;
    state->xer = xer;
...
void emul_ppc_load_state(emul_ppc_state *state)
{
    memcpy(r, state->r, sizeof(r));
    memcpy(fr, state->fr, sizeof(fr));
    lr = state->lr;
    ctr = state->ctr;
    xer = state->xer;
    fpscr = state->fpscr;
...
uint8 ReadMacInt8(uint32 addr)
{
    return *((uint8 *)ram + addr);
}

uint16 ReadMacInt16(uint32 addr)
{
    return bswap_16(*(uint16 *)((uint8 *)ram + addr));
}

uint32 ReadMacInt32(uint32 addr)
{
    if (addr > ram_size) {
        fprintf(stderr, "ReadMacInt32: address outside RAM: %d / %08X (swapped %0d / %08X)\n", addr, addr, bswap_32(addr), bswap_32(addr));
        QuitEmulator();
    }
    return bswap_32(*(uint32 *)((uint8 *)ram + addr));
}

uint64 ReadMacInt64(uint32 addr)
{
    return bswap_64(*(uint64 *)((uint8 *)ram + addr));
}

void WriteMacInt8(uint32 addr, uint8 val)
{
    *((uint8 *)ram + addr) = val;
}

void WriteMacInt16(uint32 addr, uint16 val)
{
    if (addr > ram_size) {
        fprintf(stderr, "WriteMac16: address outside RAM: %d / %08X (swapped %0d / %08X)\n", addr, addr, bswap_32(addr), bswap_32(addr));
        QuitEmulator();
    }

    *(uint16 *)((uint8 *)ram + addr) = bswap_16(val);
}

void WriteMacInt32(uint32 addr, uint32 val)
{
    if (addr > ram_size) {
        fprintf(stderr, "WriteMac32: address outside RAM: %d / %08X (swapped %0d / %08X)\n", addr, addr, bswap_32(addr), bswap_32(addr));
        QuitEmulator();
    }

    *(uint32 *)((uint8 *)ram + addr) = bswap_32(val);
}

void WriteMacInt64(uint32 addr, uint64 val)
{
    if (addr > ram_size) {
        fprintf(stderr, "WriteMac32: address outside RAM: %d / %08X (swapped %0d / %08X)\n", addr, addr, bswap_32(addr), bswap_32(addr));
        QuitEmulator();
    ...
    *(uint64 *)((uint8 *)ram + addr) = bswap_64(val);
}
...
static struct rec_step log[LOG_SIZE];
...
static void emul19(uint32 op)
{
    ...
    switch (exop) {
        ...
        case 16: {    // bclr
            ...
            // FIXME: remove this
            if (lr == 0xDEADBEEF) {
                printf("Program jumped to magic dead beef, exiting with some grace.\n");
                QuitEmulator();
            }
            pc = lr & 0xfffffffc;
blr_nobranch:
...
uint32 emul_ppc()
{
    ...
    for (;;) {
        ...
        switch (primop) {
            ...
            case 6: {      // custom import call trap
                //printf("Extended opcode %08x at %08x (68k pc %08x)\n", op, pc-4, r[24]);
                uint32 idx = (op & 0x3FFFFFF);
                emul_ppc_save_state(&state);
                run_import(idx, &state);
                emul_ppc_load_state(&state);
                break;
            }
...
bc_nobranch:
            ...
            default:
                printf("Illegal opcode %08x at %08x (prim %d)\n", op, pc-4, primop);
                dump();
                break;
        }
...
void init_emul_ppc(uint32 start, uint32 toc)
{
    ...
    // FIXME: throwing random stack size here
    r[1] = ram_size - 0x10000;
    lr = 0xDEADBEEF;
